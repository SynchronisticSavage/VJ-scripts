...
gst-launch-1.0 -e -v ximagesrc use-damage=$DAM show-pointer=$POINTER startx=$STARTX starty=$STARTY endx=$ENDX endy=$ENDY ! queue ! video/x-raw,width=$OWIDTH,height=$OHEIGHT ! videoscale ! videoconvert ! video/x-raw,format=I420, width=$WIDTH,height=$HEIGHT,framerate=30/1 ! queue ! $ENC $BIT ! $MUX name=mux ! filesink location=$VPATH/$TIME.$OFILE $ASRC ! queue2 ! audioconvert ! 'audio/x-raw,rate=44100,channels=2' ! mux.
