...
#set URI of vid example:
#VID=/path/to/videofile.avi
...
gst-launch-0.10 uridecodebin name=dec uri=$1  ! queue ! autoaudiosink dec. ! queue ! videoscale ! videorate ! ffmpegcolorspace ! video/x-raw-yuv,width=$WIDTH,height=$HEIGHT,framerate=$FPS,format=\(fourcc\)YUY2 !  v4l2sink device=$DEV
